...
const menus: Record<string, MenuItem[]> = {
  Products: [
    ...
    { title: "Brixs Wallet", subtitle: "Send, receive, and hold assets.", icon: <Wallet size={20} strokeWidth={1.5} />, href: "https://wallet.brixs.network", external: true },
    { title: "Portal", subtitle: "Bridge, swap, and manage assets.", icon: <Layers size={20} strokeWidth={1.5} />, href: "https://wallet.brixs.network", external: true },
  ],
  Solutions: [
  ...
  return ( 
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-0 h-[72px] text-[#0F1115]" onMouseLeave={() => setActiveMenu(null)}> 
      ...
      <div className="flex items-center gap-4"> 
        ...
        <Link className="hidden md:flex bg-[#0052FF] text-white hover:bg-blue-700 px-5 py-2.5 rounded-none text-[15px] font-semibold items-center gap-2 transition-colors" href="https://wallet.brixs.network"> Launch App </Link> 
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-[72px] z-40 bg-white overflow-y-auto lg:hidden flex flex-col">
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 flex gap-3 z-50">
            ...
            <Link className="flex-1 bg-[#0052FF] text-white text-[15px] font-bold text-center py-3 rounded-none" href="https://wallet.brixs.network" onClick={() => setMobileMenuOpen(false)}>Launch App</Link>
          </div>
        </div>
      )}
    </header> 
